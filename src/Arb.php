...
{
    /**
     * @var string $success_url The URL to redirect to after a successful payment.
     */
    protected string $success_url = 'http://localhost:8000/arb/response';

    /**
     * @var string $error_url The URL to redirect to after a failed payment.
     */
    protected string $error_url = 'http://localhost:8000/arb/response';

    /**
     * @var array $data The data to be sent with the payment request.
     */
    protected array $data = [];

    /**
     * @var Card|null $card The card details for the payment.
     */
    ...
    public function handlePaymentRequest(string $data): object
    {
        ...
        if ($response["status"] == "1") {
            list($paymentID, , $baseURL) = explode(":", $response["result"]);
